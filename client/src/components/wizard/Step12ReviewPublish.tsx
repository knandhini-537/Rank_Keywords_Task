...
import { Rocket, CheckCircle2, Award, Sparkles, Edit3, ArrowRight, Eye, AlertCircle } from 'lucide-react';
...
export const Step12ReviewPublish: React.FC = () => {
      ...
      <div className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-200 shadow-2xl text-center space-y-6 max-w-xl mx-auto animate-fade-in">
        ...
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs text-slate-600 space-y-1 text-left">
          <div className="font-bold text-slate-900 flex justify-between">
            ...
            <span className="text-rose-600 font-bold uppercase">{formData.listingPackage} Package</span>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs divide-y divide-slate-100">
              <div className="flex justify-between items-center pt-2">
                <span className="text-slate-500 font-medium">Package & Purpose</span>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-slate-900">{formData.listingPackage}</span>
                  <span className="text-slate-400">•</span>
                  <span className="font-bold text-rose-600">{formData.purpose} ({formData.category})</span>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setCurrentStep(5)} className="text-slate-400 hover:text-rose-600">
                    <Edit3 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-bold text-slate-900">
                    {formData.propertyInfo.bedrooms || 2} BHK • {formData.propertyInfo.carpetArea || 1200} sq.ft
                  </span>
                  <button onClick={() => setCurrentStep(6)} className="text-slate-400 hover:text-rose-600">
                    <Edit3 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setCurrentStep(7)} className="text-slate-400 hover:text-rose-600">
                    <Edit3 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-slate-500 font-medium">Photos Uploaded</span>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-slate-900">{formData.media.length} Photos</span>
                  <button onClick={() => setCurrentStep(9)} className="text-slate-400 hover:text-rose-600">
                    <Edit3 className="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
